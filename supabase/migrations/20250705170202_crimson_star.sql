/*
  # Fix webshop orders status and add VAT percentage to products

  1. New Features
    - Add VAT percentage field to products table
    - Fix webshop orders to automatically set status to 'confirmed' when payment_status is 'paid'
    - Update POS system to use retail_price instead of cost_per_unit
  
  2. Changes
    - Modify production_from_webshop_order function to ensure orders are properly confirmed
    - Add vat_percentage column to products table
    - Update existing trigger to handle payment status changes correctly
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE products ADD COLUMN vat_percentage numeric(5,2) DEFAULT 27.0;
  END IF;
END $$;
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND NEW.payment_status = 'paid' AND OLD.payment_status != 'paid') THEN
    -- Always set status to 'confirmed' when payment_status is 'paid'
...
-- Update any existing paid orders to have confirmed status
