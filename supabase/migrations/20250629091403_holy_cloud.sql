...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_vehicle_damage_reports_vehicle_id'
  ) THEN
    CREATE INDEX idx_vehicle_damage_reports_vehicle_id ON vehicle_damage_reports(vehicle_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_vehicle_damage_reports_reporter_id'
  ) THEN
    CREATE INDEX idx_vehicle_damage_reports_reporter_id ON vehicle_damage_reports(reporter_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_vehicle_damage_reports_status'
  ) THEN
    CREATE INDEX idx_vehicle_damage_reports_status ON vehicle_damage_reports(status);
  END IF;
END
$$;
...
DROP POLICY IF EXISTS "Users can view their own damage reports" ON vehicle_damage_reports;
DROP POLICY IF EXISTS "Users can insert their own damage reports" ON vehicle_damage_reports;
DROP POLICY IF EXISTS "Admins can manage all damage reports" ON vehicle_damage_reports;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_vehicle_damage_reports_updated_at'
  ) THEN
