...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM auth.users WHERE email = create_partner_user.email
  ) INTO user_exists;
  
  IF user_exists THEN
    -- Get existing user ID
    SELECT id INTO new_user_id FROM auth.users WHERE email = create_partner_user.email;
  ELSE
  ...
  IF NOT EXISTS (
    ...
    WHERE user_id = new_user_id AND partner_id = create_partner_user.partner_id
  ) THEN
