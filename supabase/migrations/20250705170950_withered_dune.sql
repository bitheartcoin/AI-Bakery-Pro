/*
  ...
  1. Changes
    - Fix the production_from_webshop_order function to properly set order status to 'confirmed'
    - Ensure orders with payment_status='paid' are automatically set to status='confirmed'
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND NEW.payment_status = 'paid' AND OLD.payment_status != 'paid') THEN
    -- IMPORTANT: Always set status to 'confirmed' when payment_status is 'paid'
